...
import framework.utils.graph.variables as vs
...
def get_all_tensor_children(tensor):
    """
    Get all tensors down stream from this one.

    For example:
      current tensor is x, which is connected to op y.
      op y has outputs q,r,s.  this function will return w, r, s
      and all tensors downstream from them.
    @param tensor:
      input tensor
    @return:
      list of tensors down stream ('children of') from the input tensor
    """
    children_list = []
    children_list.append(tensor)
    if tensor.op:
